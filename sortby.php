...
<html>
  ...
  <body>
    ...
    <div class="container my-4">
      <p class="text-center fs-4 fw-bold text-dark">Product Lists</p>
      <?php
        include 'dbconnect.php';

        if(isset($_POST['sortby'])){
          $sort = mysqli_real_escape_string($con, $_POST['sortp']);
          if($sort){
            if($sort == "lh"){
              $sortitems = "select * from product order by sellprice asc";
            }else{
              $sortitems = "select * from product order by sellprice desc";
            }
            $sortquery = mysqli_query($con, $sortitems);
            $sortresult = mysqli_num_rows($sortquery);
            if($sortresult){
              ?><ul class="cards"> <?php
              while($product = mysqli_fetch_array($sortquery)){
                ?>
                  <li class="cards_item">
                    <div class="card_show">
